...
s3_all <- arrow::s3_bucket(paste0(config$forecasts_bucket, "/bundled-summaries/project_id=", config$project_id), endpoint_override = config$endpoint, anonymous = TRUE)

forecasts <- arrow::open_dataset(s3_all) 
n_forecasts <- forecasts |> distinct(model_id, reference_date) |> collect() |> nrow()
n_models <- forecasts |> distinct(model_id) |> collect() |> nrow()
