...
3)  Write the forecast to a file.
...
Forecasts require you to make a assessment of your confidence in the prediction of the future.  You can represent your confidence (i.e., uncertainty in forecast) using a distribution or numerically using an ensemble (or sample) of predictions. 

### Any model drivers/covariates/features are welcome

You can use any data as inputs.  This includes all the targets data available to date.  All sensor-based target data is available in near-real time (< 5 day latency).  You may want to use the updated target data to re-train a model or use in data assimilation.

As this is a genuine forecasting challenge, you will need forecasted drivers if your model uses drivers as inputs.  If you are interested in using forecasted meteorology, we have developed R functions that provide access to the NOAA Global Ensemble Forecasting model forecasts for each [NEON site](https://projects.ecoforecast.org/neon4cast-docs/Shared-Forecast-Drivers.html).

### Forecasts horizon depends on the variable forecasted

Forecasts can be submitted 1 day to 1 year ahead.  The sensor-based targets are up to 30-days ahead while the community and population targets are 1 year ahead.

### Forecasts can be submitted everyday

Since forecasts can be submitted everyday, automation is important.  We provide an example GitHub repository that can be used to automate your forecast with GitHub Actions.  It also includes the use of a custom Docker Container [eco4cast/rocker-neon4cast:latest](https://github.com/eco4cast/neon4cast-ci/blob/main/Dockerfile) that has many of the packages and functions needed to generate and submit forecasts. 
...
All forecasts are scored daily using new data until the full horizon of the forecast has been scored. Forecasts are scored using the continuous ranked probability score (CRPS) via the `crps` function in the [`scoringRules`](https://cran.r-project.org/web/packages/scoringRules/index.html) R package. 
