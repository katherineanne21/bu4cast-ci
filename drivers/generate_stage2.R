...
s3_stage2 <- s3_read$path("challenges/project_id=bu4cast/drivers/stage2")
...
if (length(missing_dates) > 0) {
  for (i in seq_along(missing_dates)) {
    ...
    s3_stage1 <- s3_read$path(
      paste0("challenges/project_id=bu4cast/drivers/stage1/reference_datetime=", missing_dates[i])
    )
