...
for (m in aquatic_models$model.id[1:2]){
  ...
  scores_sites <- append(scores_sites,  get_site_coords(theme = 'vera4cast_daily', bucket = NULL, m_id = m)[[2]])
...
write.csv(scores_sites_df, 'stac/vera4cast_daily/scores/all_scores_sites.csv', row.names = FALSE)
