...
}else if(registered_models$`Does your forecast uncertainty from measurement noise?`[i] == "Yes and the noise was not estimated from data"){
  metadata$uncertainty$obs_error$present <- TRUE
  metadata$uncertainty$obs_error$data_driven <- FALSE
  metadata$uncertainty$obs_error$progagates$type <- progagates_method
}else if(registered_models$`Does your forecast uncertainty from measurement noise?`[i] == "No"){
