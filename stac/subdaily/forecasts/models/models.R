...
s3_df <- get_grouping(inv_bucket = 'neon4cast-forecasts', theme = "vera4cast-subdaily")
...
for (m in aquatic_models$model.id[1:2]){
  ...
  forecast_sites <- append(forecast_sites,  get_site_coords(theme = 'vera4cast_subdaily', bucket = NULL, m_id = m)[[2]])
  ...
  if (m %in% neon_docs$model.id){
    ...
    build_model(model_id = neon_docs$model.id[idx],
                ...
                destination_path = "stac/vera4cast_subdaily/forecasts/models/",
                description_path = "stac/vera4cast_subdaily/forecasts/models/asset-description.Rmd", # MIGHT REMOVE THIS
                aws_download_path = 'neon4cast-forecasts/parquet/vera4cast_subdaily', # CHANGE THIS BUCKET NAME
    ...
    build_model(model_id = m,
                ...
                aws_download_path = 'neon4cast-forecasts/parquet/vera4cast_subdaily',
                theme_title = "Forecasts",
                collection_name = 'forecasts',
                thumbnail_image_name = 'latest_forecast.png',
                table_schema = theme_df,
...
write.csv(forecast_sites_df, 'stac/vera4cast_subdaily/forecasts/all_forecast_sites.csv', row.names = FALSE)
