...
paths <- duckdbfs::open_dataset(glue::glue("s3://anonymous@neon4cast-inventory/neon4cast-scores?endpoint_override=sdsc.osn.xsede.org")) |>
  collect()
...
s3_df <- get_grouping(inv_bucket = 'neon4cast-scores', theme = "vera4cast-daily")
...
for (m in aquatic_models$model.id[1:2]){
  ...
  model_var_site_info <- generate_vars_sites(m_id = m, theme = 'vera4cast_subdaily')
  # print(model_var_site_info[[1]])
  # print(model_var_site_info[[2]])

  scores_sites <- append(scores_sites,  get_site_coords(theme = 'vera4cast_subdaily', bucket = NULL, m_id = m)[[2]])
  ...
  if (m %in% neon_docs$model.id){
    ...
    build_model(model_id = neon_docs$model.id[idx],
                ...
                destination_path = "stac/vera4cast_subdaily/scores/models/",
                description_path = "stac/vera4cast_subdaily/scores/models/asset-description.Rmd",
                aws_download_path = 'neon4cast-scores/parquet/vera4cast_subdaily',
                theme_title = "Scores",
    ...
    build_model(model_id = m,
                ...
                site_values = model_var_site_info[[2]],
                model_documentation = neon_docs,
                destination_path = "stac/vera4cast_daily/scores/models/",
                description_path = "stac/vera4cast_daily/scores/models/asset-description.Rmd",
                aws_download_path = 'neon4cast-scores/parquet/vera4cast_subdaily',
                theme_title = "Scores",
                collection_name = 'scores',
                thumbnail_image_name = 'latest_scores.png',
...
write.csv(scores_sites_df, 'stac/vera4cast_subdaily/scores/all_scores_sites.csv', row.names = FALSE)
