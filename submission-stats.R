...
d3 <- s3_bucket("bio230014-bucket01/challenges/scores/bundled-parquet/project_id=neon4cast", endpoint_override = "sdsc.osn.xsede.org", anonymous = TRUE)
all_results <- open_dataset(d3)
df <- all_results |>
...
d1 <- d1 |>
  mutate(type = paste0("(b) Total forecast-observation pairs: ", prettyNum(df1_sum$sum, big.mark = ",", scientific = FALSE)),
         tag = "(b)")
...
d2 <- d2 |>
  mutate(type = paste0("(a) Total forecast submssions: ", prettyNum(df2_sum$sum, big.mark = ",", scientific = FALSE)),
         tag = "(a)")
...
combined<- bind_rows(d1, d2, d3) |>
  ...
  mutate(type = factor(type, levels = c(paste0("(a) Total forecast submssions: ", prettyNum(df2_sum$sum, big.mark = ",", scientific = FALSE)), paste0("(b) Total forecast-observation pairs: ", prettyNum(df1_sum$sum, big.mark = ",", scientific = FALSE)), paste0("(c) Total unique models: ", prettyNum(df3_sum$sum, big.mark = ",", scientific = FALSE))))) |>
  ggplot() +
...
all_results <- arrow::open_dataset("s3://anonymous@bio230014-bucket01/challenges/forecasts/bundled-parquet/project_id=neon4cast/duration=P1D/variable=temperature/model_id=flareGLM?endpoint_override=sdsc.osn.xsede.org")
df4 <- all_results |>
...
df_target <- df |>
  filter(datetime < as_datetime("2025-01-01 00:00:00"),
         !is.na(observation)) |>
  summarize(count = n())

d3 <- arrow::s3_bucket("bio230014-bucket01/challenges/scores/bundled-parquet/project_id=neon4cast", endpoint_override = "sdsc.osn.xsede.org", anonymous = TRUE)
all_results <- arrow::open_dataset(d3)
df_scores <- all_results |>
  filter(duration %in% c("P1D", "P1W")) |>
  filter(reference_datetime >= as_datetime("2021-01-01 00:00:00") &
